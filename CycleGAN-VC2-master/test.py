...
class CycleGANTraining(object):
    def __init__(self,
        ...
        self.validation_num = 2000
        self.cut_len = 1800000

        self.generator_loss = torch.tensor(0)
        self.d_loss = torch.tensor(0)

        if gen_model_dir is not None:
            path = os.listdir(gen_model_dir)[-1]
            path = os.path.join(gen_model_dir, path)
            self.start_epoch = self.loadModel(path)
        ...
        for epoch in range(self.start_epoch, self.num_epochs):
            ...
            if chunk_size_A > chunk_size_B:
                ...
                for data_A_, data_B_ in zip(self.dataset_A, self.dataset_B):
                    ...
                    mini_epoch += 1
                    if mini_epoch % 10 == 0:
                        self.save_test(epoch, start_time_epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gen', type=str,
                        help="gen model", default=gen_model_default)
    ...
    assert argv.disc is not None
    disc_model = os.path.join(argv.disc, 'model_checkpoint')

    assert argv.gen is not None
    gen_model = os.path.join(argv.gen, 'model_checkpoint')
