...
def load_process(train_dir, class_id):
    i = 0
    wavs = list()
    run_i = len(os.listdir(train_dir)) // thread_num
    thread_list = list()
...
if __name__ == '__main__':
    ...
    i = 0
    wavs = list()
    run_i = len(os.listdir(train_A_dir)) // thread_num
    thread_list = list()
    ...
    log_f0s_mean_A, log_f0s_std_A = preprocess_mine.logf0_statistics(f0s=f0s)
    coded_sps_norm, coded_sps_mean_A, coded_sps_std_A = preprocess_mine.coded_sps_normalization_fit_transform(
        coded_sps=coded_sps)
...
#############################################
    i = 0
    wavs = list()
    run_i = len(os.listdir(train_B_dir)) // thread_num
    thread_list = list()
    ...
    log_f0s_mean_B, log_f0s_std_B = preprocess_mine.logf0_statistics(f0s=f0s)
    coded_sps_norm, coded_sps_mean_B, coded_sps_std_B = preprocess_mine.coded_sps_normalization_fit_transform(
        coded_sps=coded_sps)
    ...
    np.savez(os.path.join(cache_folder, 'mcep_normalization.npz'),
             mean_A=coded_sps_mean_A,
             std_A=coded_sps_std_A,
             mean_B=coded_sps_mean_B,
             std_B=coded_sps_std_B)